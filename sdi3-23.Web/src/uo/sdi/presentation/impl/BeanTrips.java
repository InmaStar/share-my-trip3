...
public class BeanTrips implements Serializable {
	...
	try {
	    TripService tripServ = BusinessFactories.services.getTripService();
	...
	for (TripDTO viaje : viajesSeleccionados) {
	    try {
		BusinessFactories.services.getTripService().cancel(viaje);
		Log.info("El viaje [%d] ha sido cancelado", viaje.getId());
	    } catch (Exception e) {
		Log.debug(
	...
	try {
	    ...
	    BusinessFactories.services.getUserService().cancelApplication(
		    usuario, viaje);
	    listadoDisponibles();
	    return "exito";
	} catch (Exception e) {
	...
	try {
	    Log.info("Se está solicitando plaza para el viaje "
		    + "[%d] al usuario [%d]", viaje.getId(), usuario.getId());
	    BusinessFactories.services.getUserService().requestSeat(usuario,
		    viaje);
	    listadoDisponibles();
	    return "exito";
	} catch (Exception e) {
	...
	List<TripDTO> listaViajes = BusinessFactories.services.getTripService()
		.findAll();
